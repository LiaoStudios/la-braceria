...
function Stars({ n }: { n: number }) {
  return (
    <div className="flex gap-1 text-straw">
...
export function Reviews() {
  ...
  return (
    <section className="bg-sand texture-paper py-24 md:py-32">
      <div className="mx-auto max-w-4xl px-6 md:px-12">
        ...
        <div className="relative min-h-[240px] rounded-[2rem] bg-white p-8 shadow-warm md:p-12">
          <Quote className="mb-4 text-ember/30" size={40} />
          ...
          <div className="mt-8 flex items-center justify-center gap-3">
            <button onClick={() => go(-1)} aria-label="Recensione precedente" className="flex h-10 w-10 items-center justify-center rounded-full border border-ink/15 transition hover:bg-ember hover:text-parchment">
              <ChevronLeft size={20} />
            </button>
            {reviews.map((_, idx) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all ${idx === i ? 'w-6 bg-ember' : 'w-2.5 bg-ink/20'}`}
              />
            ))}
            <button onClick={() => go(1)} aria-label="Recensione successiva" className="flex h-10 w-10 items-center justify-center rounded-full border border-ink/15 transition hover:bg-ember hover:text-parchment">
        ...
        <div className="mt-8 text-center">
          <a href={siteInfo.social.google} target="_blank" rel="noopener noreferrer" className="text-sm font-bold uppercase tracking-widest text-ember hover:text-ember-deep">
