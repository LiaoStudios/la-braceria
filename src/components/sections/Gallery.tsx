        ...
        {galleryCategories.map((c) => (
          <button
            ...
            className={`rounded-full px-5 py-2 text-sm font-medium transition ${
              filter === c ? 'bg-ember text-parchment shadow-warm' : 'bg-white text-ink/70 hover:bg-sand'
            }`}
          >
            {c}
          </button>
      ...
      {hasMore && (
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-ink/15 bg-white px-7 py-3.5 text-sm font-bold uppercase tracking-wider text-ink transition hover:bg-sand"
          >
            <Images size={18} />
